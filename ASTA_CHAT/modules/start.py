...
@app.on_message(
    filters.regex(r"^/start(?:@\w+)?(?:\s+.*)?$")
)
async def start_command(
    client,
    message: Message,
):
    """Handle /start reliably in private chats and groups.

    A regex is used instead of filters.command() so Telegram variants such as
    /start@BotUsername and /start <payload> are handled consistently.
    """

    try:
        if not message.from_user:
            print("[START] Ignored message without from_user.")
            return
        ...
        try:
            ...
            print(f"[START] Panel sent to user_id={user_id}")
            return

        except Exception as photo_error:
            print(
                f"[START] Photo failed: "
                f"{type(photo_error).__name__}: {photo_error}"
            )

        # Fallback if Telegram cannot fetch/send the start image.
        try:
            await message.reply_text(
                text,
                reply_markup=buttons,
            )
            print(f"[START] Text fallback sent to user_id={user_id}")

        except Exception as text_error:
            print(
                f"[START] Text failed: "
                f"{type(text_error).__name__}: {text_error}"
            )

    except Exception as error:
        print(
            f"[START] Handler failed: "
